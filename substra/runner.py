...
def _docker_build(docker_client, dockerfile_path, name, rm=False):
    ...
    start = time.time()
    docker_client.images.build(path=dockerfile_path, tag=name, rm=rm)
...
def _docker_run(docker_client, name, command, volumes, remove=True):
    ...
    try:
        ...
        docker_client.containers.run(
            name, command=command, volumes=volumes, remove=remove, user=USER, userns_mode="host"
        )
...
def compute_train(
    ...
    _docker_run(docker_client, DOCKER_ALGO_TAG, command=command, volumes=volumes)
...
def compute_perf(
    ...
    if n_fake_samples:
        command += f" --n-fake-samples {n_fake_samples}"
    _docker_run(docker_client, DOCKER_METRICS_TAG, command=command, volumes=volumes)
...
def raise_if_path_traversal(requested_paths, to_directory, archive_path):
    ...
    for requested_path in requested_paths:
        ...
        if is_traversal:
            raise PathTraversalException(archive_path, requested_path.replace(to_directory, ""))
...
def _extract_archive(archive_path, to_directory):
    if zipfile.is_zipfile(archive_path):
        with zipfile.ZipFile(archive_path, "r") as zf:
            ...
            filenames = [os.path.join(to_directory, filename) for filename in zf.namelist()]
        ...
        with tarfile.open(archive_path, "r:*") as tf:
            ...
            # Check no path traversal
            filenames = [os.path.join(to_directory, filename) for filename in tf.getnames()]
            raise_if_path_traversal(filenames, to_directory, archive_path)
...
def _compute(
    ...
    print(
        f"Evaluating performance - compute metric with {test_pred_path} "
