...
class TestFixtures(TestCase):
    ...
    def tearDown(self):
        try:
            os.remove('/tmp/.substra_e2e_multi_orgs')
        except:
            pass
    ...
    def test_add_fixtures(self):
        ...
        output = popen(['substra', 'add', 'data-manager', '--profile=chunantes', '--config=/tmp/.substra_e2e_multi_orgs', data], stdout=PIPE).communicate()[0]
        ...
        # create data_manager, test data and challenge on owkin
        data = json.dumps({
        ...
        output = popen(['substra', 'add', 'data-manager', '--profile=owkin', '--config=/tmp/.substra_e2e_multi_orgs', data], stdout=PIPE).communicate()[0]
        res = output.decode('utf-8')
        ...
        output = popen(['substra', 'add', 'algo', '--profile=chunantes', '--config=/tmp/.substra_e2e_multi_orgs', data], stdout=PIPE).communicate()[0]
        res = output.decode('utf-8')
        res = json.loads(res)
        ...
        output = popen(['substra', 'add', 'traintuple', '--profile=chunantes', '--config=/tmp/.substra_e2e_multi_orgs', data], stdout=PIPE).communicate()[0]
        res = output.decode('utf-8')
